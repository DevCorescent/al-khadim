/**
 * One-off / idempotent: seed the 16 transactional EmailTemplate rows that the
 * platform's route handlers now render via sendTemplatedMail() instead of
 * building HTML inline. Safe to re-run — existing rows (including any admin
 * edits made since) are left untouched; only missing slugs are created.
 * Run with:  node scripts/seedEmailTemplates.js
 */
...
async function seedEmailTemplates() {
  for (const t of TEMPLATES) {
    await prisma.emailTemplate.upsert({
      where: { slug: t.slug },
      update: {}, // don't clobber admin edits on re-run — only create if missing
      create: t,
    });
  }
