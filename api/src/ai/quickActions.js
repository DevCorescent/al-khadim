...
/**
 * @param {string[]} toolNames - tools actually called during this turn, in call order
 * @param {number} limit
 * @returns {string[]} up to `limit` unique, curated follow-up questions
 */
function getQuickActionsForTools(toolNames, limit = 4) {
  const seen = new Set();
  const out = [];
...
module.exports = { getQuickActionsForTools };
