// Ported from api/src/ai/tools/finance.js
/**
...
const tools: AiTool[] = [
  {
    ...
    handler: async (args) => {
      const where: any = { docType: args.docType || 'INVOICE' };
  ...
  },
  {
    ...
    handler: async (args) => {
      ...
      const from = args.fromDate ? new Date(args.fromDate) : new Date(now.getFullYear(), 0, 1);
      const to = args.toDate ? new Date(args.toDate) : new Date(now.getFullYear(), 11, 31, 23, 59, 59);
      const where: any = { date: { gte: from, lte: to }, status: { notIn: ['DRAFT', 'REJECTED'] } };
