// Ported from api/src/ai/tools/public.js
/**
...
const tools: AiTool[] = [
  {
    ...
    allowedRoles: ['PUBLIC'],
    handler: async (args) => {
      const where: any = { status: 'OPEN', isPublished: true };
      ...
      const take = Math.min(Math.max(1, Math.floor(Number(args.limit)) || 20), MAX_ROWS);
      ...
      return {
        summary: `${total} open job${total === 1 ? '' : 's'} found${total > jobs.length ? ` (showing the most recent ${jobs.length} of ${total})` : ''}.`,
        table,
      };
    },
  },
